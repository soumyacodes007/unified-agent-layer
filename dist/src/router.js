...
export const GROQ_MODEL_ALIASES = {
...
function normalizeMessagePart(part) {
    ...
    if (typeof maybePart.input_text === 'string')
        return maybePart.input_text;
...
function normalizeMessageContent(content) {
    ...
    if (Array.isArray(content))
        return content.map(normalizeMessagePart).filter(Boolean).join('\n');
    return '';
}
...
function getAutoRoute(messages) {
...
export function listGroqChatModels() {
    return Object.values(GROQ_CHAT_MODELS);
}
export function resolveGroqChatModel(model) {
    const normalized = GROQ_MODEL_ALIASES[model] ?? model;
    return GROQ_CHAT_MODELS[normalized] ?? null;
}
export function routeGroqChatModel(messages, requestedModel = 'auto') {
...
export function supportsReasoningFormat(modelId) {
    return Boolean(GROQ_CHAT_MODELS[modelId]?.supportsReasoningFormat);
}
export function getAllowedReasoningEfforts(modelId) {
    return GROQ_CHAT_MODELS[modelId]?.reasoningEfforts ?? [];
}
export async function listLiveGroqModels() {
