...
router.post('/v1/compute', async (req, res) => {
    ...
    async function tryExecute(url, isFallback) {
        try {
            // First fetch available runtimes to validate language + get version
            const runtimesRes = await fetch(`${url}/api/v2/runtimes`);
            if (!runtimesRes.ok)
                throw new Error('Piston runtimes endpoint unavailable');
            const runtimes = await runtimesRes.json();
            const runtime = runtimes.find(r => r.language === language.toLowerCase() || r.aliases.includes(language.toLowerCase()));
            if (!runtime) {
                res.status(400).json({
