...
function isJsonResponseFormat(responseFormat: unknown): boolean {
...
router.post('/chat', async (req: Request, res: Response) => {
  ...
  try {
    const routing = routeGroqChatModel(messages as ChatMessage[], model.trim());
    ...
    if (reasoning_effort !== undefined) {
      ...
      if (!allowed.includes(reasoning_effort as GroqReasoningEffort)) {
        res.status(400).json({
    ...
    if (reasoning_format !== undefined) {
      ...
      const allowedFormats: GroqReasoningFormat[] = ['parsed', 'raw', 'hidden'];
      if (!allowedFormats.includes(reasoning_format as GroqReasoningFormat)) {
        res.status(400).json({
    ...
    if (top_p !== undefined) completionRequest.top_p = toFiniteNumber(top_p, 1);
    if (seed !== undefined) completionRequest.seed = toPositiveInt(seed, 0);
    if (stop !== undefined) completionRequest.stop = stop;
    if (response_format !== undefined) completionRequest.response_format = response_format;
    if (tools !== undefined) completionRequest.tools = tools;
    if (tool_choice !== undefined) completionRequest.tool_choice = tool_choice;
    if (reasoning_effort !== undefined) completionRequest.reasoning_effort = reasoning_effort;
    if (include_reasoning !== undefined) completionRequest.include_reasoning = include_reasoning;
    if (reasoning_format !== undefined) completionRequest.reasoning_format = reasoning_format;

    const completion = await getGroq().chat.completions.create(completionRequest as any);
